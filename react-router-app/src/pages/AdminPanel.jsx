...
function AdminPanel() {
    ...
    const fetchData = async () => {
        try {
            const uRes = await fetch('https://qadam-backend-x1d2.onrender.com/api/users', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if(uRes.ok) setUsers(await uRes.json());

            const cRes = await fetch('https://qadam-backend-x1d2.onrender.com/api/courses');
            if(cRes.ok) setCourses(await cRes.json());

            const mRes = await fetch('https://qadam-backend-x1d2.onrender.com/api/messages', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    const toggleRole = async (id, currentRole) => {
        ...
        try {
            ...
            const res = await fetch(`https://qadam-backend-x1d2.onrender.com/api/users/${id}/role`, {
                method: 'PUT',
                headers: { 
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const handleStartChat = async (userId) => {
        ...
        try {
            const res = await fetch(`https://qadam-backend-x1d2.onrender.com/api/messages/admin`, {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
    ...
    const handleReply = async (id) => {
        ...
        try {
            const res = await fetch(`https://qadam-backend-x1d2.onrender.com/api/messages/${id}/reply`, {
                method: 'PUT',
                headers: { 
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
